...
def nan_to_num(X, y, df, nan_mode):
    if nan_mode is None:
    ...
    elif nan_mode == "local_avg":
        for i in range(60):
            temp = X[:, :, max(0, i - 3):min(60, i + 4)]
            temp = np.nanmean(temp, axis=2)
            X[:, :, i] = np.where(np.isnan(X[:, :, i]), temp, X[:, :, i])
        X, y, df = nan_to_num(X, y, df, "avg")
    elif nan_mode == "skip":
        pass
    return X, y, df
